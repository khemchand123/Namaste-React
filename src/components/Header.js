    ...
    <div className="flex flex-1 justify-start h-28 border-solid border border-gray-500 shadow-lg">
      ...
      <div className="flex flex-auto justify-around">
        <ul className="flex flex-wrap h-28 content-center">
          ...
          <li className="mx-3 p-1 hover:bg-gray-200 hover:rounded-md">Cart</li>
          <li className="mx-3 p-1 hover:bg-gray-200 hover:rounded-md3">
            <Link to="/swiggy-instamart">Swiggy-Instamart</Link>
          </li>
        </ul>
        <div className="flex content-center">
          <span className="m-auto p-2 border border-s-2">{user.name} - {user.email}</span>
          {isLoginIn ? (
            <button className="w-28 border border-s-2 p-3 m-auto bg-green-600 rounded-2xl text-white" onClick={() => setIsLoginIn(false)}>LogOut</button>
          ) : (
            <button className="w-28 border border-s-2 p-3 m-auto bg-red-600 rounded-2xl text-white" onClick={() => setIsLoginIn(true)}>LogIn</button>
          )}
        </div>
      </div>
    </div>
